...
for parcel in cargo1_list:
    ...
    for spacecraft in list_spacecrafts:
        ...
        spacecraft["kg/m3"] = ratio
    list_spacecraft = []
    list_spacecraft = sorted(list_spacecrafts, key=lambda spacecraft: spacecraft["kg/m3"])
    if(parcel["mass"] <= list_spacecraft[0]["mass"]) and (parcel["volume"] <= list_spacecraft[0]["volume"]):
        list_spacecraft[0]["mass"] -= parcel["mass"]
        list_spacecraft[0]["volume"] -= parcel["volume"]
        counter1 += 1
    elif(parcel["mass"] <= list_spacecraft[1]["mass"]) and (parcel["volume"] <= list_spacecraft[1]["volume"]):
        list_spacecraft[1]["mass"] -= parcel["mass"]
        list_spacecraft[1]["volume"] -= parcel["volume"]
        counter2 += 1
    elif(parcel["mass"] <= list_spacecraft[2]["mass"]) and (parcel["volume"] <= list_spacecraft[2]["volume"]):
        list_spacecraft[2]["mass"] -= parcel["mass"]
        list_spacecraft[2]["volume"] -= parcel["volume"]
        counter3 += 1
    elif(parcel["mass"] <= list_spacecraft[3]["mass"]) and (parcel["volume"] <= list_spacecraft[3]["volume"]):
        list_spacecraft[3]["mass"] -= parcel["mass"]
        list_spacecraft[3]["volume"] -= parcel["volume"]
        counter4 += 1
    else:
